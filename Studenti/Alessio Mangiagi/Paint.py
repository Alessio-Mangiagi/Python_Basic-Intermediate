...
class Paint(tk.Tk):
    ...
    def Initialize_color_picker(self):
        color = colorchooser.askcolor(title="Seleziona un colore")
    
    # Se un colore è stato selezionato (non è None)
        if color[1] is not None:
            self.line_color = color[1]  # Imposta il colore selezionato
            print(f"Colore selezionato: {self.line_color}")
