...
# Funzione per creare una finestra per l'operazione selezionata
...
# Creazione dei widget per l'input dei numeri e il risultato
    tk.Label(finestra, text="Primo numero:").pack()
    primo_numero = tk.Entry(finestra,width=5)
    primo_numero.pack()
    tk.Label(finestra, text="Secondo numero:").pack()
    secondo_numero = tk.Entry(finestra,width=5)
    secondo_numero.pack()

    risultato_label = tk.Label(finestra, text="Risultato: ")
    risultato_label.pack()

# Creazione dei pulsanti per le operazioni
    def calcola():
        try:
...
tk.Label(root, text="Scegli un'operazione:").pack()
