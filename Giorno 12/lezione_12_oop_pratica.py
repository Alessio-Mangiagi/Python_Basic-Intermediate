...
class app_esercitazione(tk.Tk):
    ...
    def Enter_Callback(self, event):
        """Callback per l'evento di entrata del mouse su un widget."""
        print("Mouse entra nel widget:", event.widget)
        self.button_info.config(bg="lightblue")

    def Leave_Callback(self, event):
        """Callback per l'evento di uscita del mouse da un widget."""
        print("Mouse esce dal widget:", event.widget)
        self.button_info.config(bg="lightgray")

    def Change_Focus_default(self, event):
        """Callback per l'evento di cambio focus su un widget senza target specificato."""
        print("Focus cambiato su:", event.widget)
        self.entry_nome.focus_set()

    def Change_Focus(self, event, target=None):
        """Callback per l'evento di cambio focus su un widget."""
        if target:
            print("Focus cambiato su:", event.widget, "-> Target:", target)
            target.focus_set()
        else:
            print("Nessun target specificato per il cambio focus")
    ...
    def inizializza_toolbar(self):
        ...
        self.button_info.bind("<Enter>", self.Enter_Callback)
        self.button_info.bind("<Leave>", self.Leave_Callback)
    ...
    def inizializza_sezione_body(self):
        ...
        self.entry_nome = tk.Entry(
            self.sezione_body, font=("Arial", 14), textvariable=self.nome_var
        )
        self.entry_nome.bind(
            "<Return>",
            lambda event: self.Change_Focus_default(event),
        )
        self.entry_nome.bind(
            "<Tab>", lambda event: self.Change_Focus(event, target=self.entry_cognome)
        )
        self.entry_nome.pack(pady=10, padx=10, fill=tk.X)

        self.label_cognome = tk.Label(
            self.sezione_body, text="Cognome:", font=("Arial", 14)
        )
        self.label_cognome.pack(pady=10, padx=10, anchor=tk.W)

        self.entry_cognome = tk.Entry(
            self.sezione_body, font=("Arial", 14), textvariable=self.cognome_var
        )
        self.entry_cognome.pack(pady=10, padx=10, fill=tk.X)

        self.entry_cognome.bind(
            "<Tab>", lambda event: self.Change_Focus(event, target=self.test_1)
        )
    ...
    def inizializza_sezione_radiobutton(self):
        ...
        self.radio_maschio = tk.Radiobutton(
            self.sezione_button,
            text="Maschio",
            variable=self.radio_genere,
            value="Maschio",
        )
        ...
        self.radio_femmina = tk.Radiobutton(
            self.sezione_button,
            text="Femmina",
            variable=self.radio_genere,
            value="Femmina",
        ...
        self.radio_altro = tk.Radiobutton(
            self.sezione_button, text="Altro", variable=self.radio_genere, value="Altro"
        )
    ...
    def inizializza_sezione_testo(self):
        ...
        self.test_1 = tk.Text(
            self.sezione_testo, font=("Arial", 14), bg="#e20a0a", height=10, width=50
        )
    ...
    def inizializza_sezione_checkbox(self):
        ...
        self.checkbox_sport = tk.Checkbutton(
            self.sezione_checkbox, text="Sport", variable=self.checkbox_sport_var
        )
        self.checkbox_musica = tk.Checkbutton(
            self.sezione_checkbox, text="Musica", variable=self.checkbox_musica_var
        )
        self.checkbox_trekking = tk.Checkbutton(
            self.sezione_checkbox, text="Trekking", variable=self.checkbox_trekking_var
        )
        self.checkbox_cucina = tk.Checkbutton(
            self.sezione_checkbox, text="Cucina", variable=self.checkbox_cucina_var
        )
        self.checkbox_paintball = tk.Checkbutton(
            self.sezione_checkbox,
            text="Paintball",
            variable=self.checkbox_paintball_var,
        )
